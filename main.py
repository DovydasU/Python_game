...
def main():
    ...
    while run:
        ...
        for ecent in pygame.event.get():
            ...
            if ecent.type == pygame.KEYDOWN:
                if ecent.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)
                    yellow_bullets.append(bullet)                 
                if ecent.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:       
...
if __name__ == "__main__":
    ...
    # Build a Web Socket with 
    # build a websocked with laravel